...
func (cc *CommentCreate) SetContext(s string) *CommentCreate {
	cc.mutation.SetContext(s)
	return cc
}
...
func (cc *CommentCreate) AddOwnerIDs(ids ...int) *CommentCreate {
	cc.mutation.AddOwnerIDs(ids...)
	return cc
}

// AddOwner adds the "owner" edges to the User entity.
func (cc *CommentCreate) AddOwner(u ...*User) *CommentCreate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
...
func (cc *CommentCreate) AddLikes(l ...*Like) *CommentCreate {
	ids := make([]int, len(l))
	for i := range l {
		ids[i] = l[i].ID
	}
...
func (cc *CommentCreate) check() error {
	if _, ok := cc.mutation.Context(); !ok {
		return &ValidationError{Name: "Context", err: errors.New(`ent: missing required field "Comment.Context"`)}
	}
	if _, ok := cc.mutation.CreatedAt(); !ok {
