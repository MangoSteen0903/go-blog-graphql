...
func (hq *HashtagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Hashtag, error) {
	...
	for name, query := range hq.withNamedPosts {
		if err := hq.loadPosts(ctx, query, nodes,
			func(n *Hashtag) { n.appendNamedPosts(name) },
			func(n *Hashtag, e *Post) { n.appendNamedPosts(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (hq *HashtagQuery) WithNamedPosts(name string, opts ...func(*PostQuery)) *HashtagQuery {
	query := &PostQuery{config: hq.config}
	for _, opt := range opts {
		opt(query)
	}
	if hq.withNamedPosts == nil {
		hq.withNamedPosts = make(map[string]*PostQuery)
	}
	hq.withNamedPosts[name] = query
	return hq
}
...
func (hgb *HashtagGroupBy) sqlScan(ctx context.Context, v any) error {
...
func (hs *HashtagSelect) sqlScan(ctx context.Context, v any) error {
