...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	for name, query := range pq.withNamedHashtags {
		if err := pq.loadHashtags(ctx, query, nodes,
...
// WithNamedHashtags tells the query-builder to eager-load the nodes that are connected to the "hashtags"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithNamedHashtags(name string, opts ...func(*HashtagQuery)) *PostQuery {
	query := &HashtagQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	if pq.withNamedHashtags == nil {
